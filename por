...
if cmd == "-S"
    ...
    puts "recommend not using this if on derivative of Debian"
    ...
    system "doas apt show #{pkgs} #{args} #{args2} #{args3} #{args4} #{args5}"
    ...
    puts"simulates the action. when using the argument it must be the 1st argument"
    puts"\n"
    puts"por -Rs [pkg]"
